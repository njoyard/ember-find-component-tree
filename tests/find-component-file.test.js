...
describe('findComponentFile', () => {
  ...
  it('finds a .gjs file', async () => {
    ...
    expect(await findComponentFile(fixture.addonPath(), importPath, PKG)).toBe(
      path.join(fixture.addonPath(), 'components/button.gjs'),
    );
  });
  ...
  it('finds a .gts file', async () => {
    ...
    expect(await findComponentFile(fixture.addonPath(), importPath, PKG)).toBe(
      path.join(fixture.addonPath(), 'components/button.gts'),
    );
  });
  ...
  it('finds a .js file with a sibling .hbs and returns the .hbs', async () => {
    ...
    expect(await findComponentFile(fixture.addonPath(), importPath, PKG)).toBe(
      path.join(fixture.addonPath(), 'components/button.hbs'),
    );
  });
  ...
  it('finds a .js file without sibling .hbs but with templates/components', async () => {
    ...
    expect(await findComponentFile(fixture.addonPath(), badgeImportPath, PKG)).toBe(
      path.join(fixture.addonPath(), 'templates/components/badge.hbs'),
    );
  });
  ...
  it('falls back to the .js file itself when no .hbs exists', async () => {
    ...
    const badgeImportPath = 'my-components/components/badge';
    expect(await findComponentFile(fixture.addonPath(), badgeImportPath, PKG)).toBe(
      path.join(fixture.addonPath(), 'components/badge.js'),
    );
  });
  ...
  it('finds a standalone .hbs file', async () => {
    ...
    expect(await findComponentFile(fixture.addonPath(), legacyImportPath, PKG)).toBe(
      path.join(fixture.addonPath(), 'components/legacy.hbs'),
    );
  });

  it('returns null when nothing is found', async () => {
    expect(await findComponentFile(fixture.addonPath(), importPath, PKG)).toBeNull();
  });
  ...
  it('resolves nested paths', async () => {
    ...
    expect(await findComponentFile(fixture.addonPath(), nestedImportPath, PKG)).toBe(
      path.join(fixture.addonPath(), 'components/form/field.gjs'),
    );
  });
});
